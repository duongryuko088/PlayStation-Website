	...
	function pages_list($count_pages,$get_page){
		...
		if($count_pages>1)
		{
			if($get_page>=1 && $get_page <= ceil($count_buttons/2))
			{
				...
				if($get_page !=1)
				{
					$pages_list .= '<a href="'.$_SERVER['PHP_SELF'].$query_string_1.'"><span class="page-button">First</span></a><a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.($get_page-1).'"><span class="page-button"><</span></a>'.$page_buttons.'<a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.($get_page+1).'"><span class="page-button">></span></a><a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.$count_pages.'"><span class="page-button">Last</span></a>';
				}
				else
				{
					$pages_list .= $page_buttons.'<a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.($get_page+1).'"><span class="page-button">></span></a><a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.$count_pages.'"><span class="page-button">Last</span></a>';
				}
			}
			...
			else if($get_page > $count_pages - ceil($count_buttons/2) && $get_page <= $count_pages)
			{
				...
				{
					$pages_list .= '<a href="'.$_SERVER['PHP_SELF'].$query_string_1.'"><span class="page-button">First</span></a><a href="'.$_SERVER['PHP_SELF'].'?'.$query_string.'page='.($get_page-1).'"><span class="page-button"><</span></a>'.$page_buttons;
				}
			}
	}
		return $pages_list;

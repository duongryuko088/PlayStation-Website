...
class M_bai_viet extends database{
		...
		if(isset($_POST['btn_them_bv']))
		{
			...
			if(isset($_FILES['hinh_bv']) && $_FILES['hinh_bv']['error'] ==0)
			{
				...
				if(in_array($type_hinh,$list_file_type))
				{
					if($_FILES['hinh_bv']['size'] <= $maxsize)
					{
						...
						{
							$this->loi_hinh = 'Erorr to load image';
							$insert = false;
						}
					}
					else
			...
			if($insert)
			{
				...
				if($this->execute($param))
					$_SESSION['them_bai_xong'] = 'Insert successfully';
				else
					$_SESSION['them_bai_xong'] = 'Erorr!Please try again';
	...
	public function sua_bai_viet($ma_bai_viet,$hinh_ht){
		if(isset($_POST['btn_them_bv']))
		{
			...
			if(isset($_FILES['hinh_bv']) && $_FILES['hinh_bv']['error'] ==0)
			{
				...
				if(in_array($type_hinh,$list_file_type))
				{
					if($_FILES['hinh_bv']['size'] <= $maxsize)
					{
						...
						{
							$this->loi_hinh = 'Error loading image';
							$update = false;
						}
					}
					else
			...
			if($update)
			{
				...
				setcookie('sua_bai_xong','Update successfully',time()+20);
	...
	public function them_loai_bai(){
		if(isset($_POST['btn_cap_nhat_loai_bv']))
		{
			...
			{
				$insert = false;
				$this->loi_trung_ten = 'This type has exists';
			}
			else if($ten_loai_bai == '' || $ten_loai_bai == ' ')
			{
				$insert = false;
				$this->loi_trung_ten = 'Enter type';
			}
			...
			if($insert)
			{
				...
				$_SESSION['them_loai_xong'] = 'Insert post type successfully';
	...
	public function sua_loai_bai($ma_loai_bai){
		if(isset($_POST['btn_cap_nhat_loai_bv']))
		{
			...
			if($update)
			{
				...
				setcookie('sua_loaibv','Update successfully',time()+20);
				header('location: '.$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
			}
		}
	}

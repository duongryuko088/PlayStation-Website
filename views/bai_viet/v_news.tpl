<div class="row  outer-bottom-vs">
	...
	<span class="review">View: {$tin->luot_xem}</span>
	<span class="date-time">{date('d-m-Y',strtotime($tin->ngay_dang))}</span>
	<p>{$tin->noi_dung_tom_tat}</p>
	<a href="bt_{thu_vien::bo_dau($tin->ten_bai_viet)}.{$tin->ma_bai_viet}.html" class="btn btn-upper btn-primary read-more">Read</a>
</div>
				{/foreach}

...
class M_nguoi_dung extends database{
	...
	public function error_trung_user(){
		if(isset($_POST["btn_dk"]))
		{
		  ...
		  {
			  return "<div style=\"color:#F00; margin:10px auto ;padding:10px 0; border:1px solid #F00; text-indent:10px\">Username has existed</div>";
		  }
		  else if(!$this->user_da_ton_tai(array(md5($_POST['mat_khau'])),'select * from nguoi_dung where mat_khau =?'))
		  {
			  return "<div style=\"color:#F00; margin:10px auto ;padding:10px 0; border:1px solid #F00; text-indent:10px\">Password has existed</div>";
		  }
		  else if(!$this->user_da_ton_tai(array(trim($_POST['email'])),'select * from nguoi_dung where email =?'))
		  {
			  return "<div style=\"color:#F00; margin:10px auto ;padding:10px 0; border:1px solid #F00; text-indent:10px\">Email has existed</div>";
		  }
		  else
			  return '';
		}
	...
	public function remove_session_login_false(){
		...
		{
			echo '<div style="color:#F00; margin:10px auto ;padding:10px 0; border:1px solid #F00; text-indent:10px"> Error : Username or Password has already existed . </div>';
	...
	public function cap_nhat_nguoi_dung(){
		if(isset($_POST['btn_cap_nhat']))
		{
		  ...
		  if($update)
		  {
			  ...
			  setcookie('sua_user','Update successfully',time() + 60*20);
	...
	public function doi_mat_khau(){
		if(isset($_POST['btn_change_pass']))
		{
			...
			if($change)
			{
				...
				$this->setQuery($sql);
				$this->execute($param);
				setcookie('doi_mat_khau_xong','Update password successfully',time() +20*60);
